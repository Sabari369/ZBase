async function logout() {
	try {
		...
		/*alert(result.message); // Show logout message*/
...
window.onload = async function() {
	...
	if (databases != null && Object.keys(databases).length > 0) {
		...
		if (Object.keys(databases).length == 0) {
			document.getElementById("noDatabaseMessage").style.display = "flex";
		} else {
	...
	//
	//  console.log(document.querySelector(".query-box"));
	//  let queryContainer = document.querySelector(".query-box");
...
function showCreateTableUI() {
	...
	let create = document.getElementById("createTableInDb");
...
function updateExplorer() {
	...
	Object.keys(databases).forEach((dbName) => {
		...
		nameArrow.onclick = async function() {
			console.log(nameArrow.classList)
			if (nameArrow.parentElement.classList.contains("currentDatabase")) {
				console.log(nameArrow.classList)
				nameArrow.parentElement.classList.remove("currentDatabase")
			}
			...
			if (!isExpanded) {
				...
				selectDatabase(dbName, nameArrow.parentElement);
				console.log(nameArrow.parentElement);
...
function selectDatabase(dbName, dbElement) {
	...
	//
	document
		.querySelectorAll(".currentDatabase")
		.forEach((el) => el.classList.remove("currentDatabase"));
	document
	...
	dbElement.classList.add("currentDatabase");
	// showCreateTableUI();
}
...
function deleteTable() {
	...
	// Create text
	let text = document.createElement("p");
	text.innerText = "Do you really want to delete this table?";
...
function operation(element) {
	...
	console.log(operationSpace.children);
	console.log(operationSpace.children.length);

	if (operationSpace.children.length > 1) {
		operationSpace.lastChild.remove();
...
async function createDatabaseFetch(createJSON) {
	...
	try {
		...
		if (result.result) {
			...
			let noDbMsg = document.querySelector(".no-db-container");
			console.log(noDbMsg);
			if (!noDbMsg || noDbMsg != null) {
...
async function insertFetch(insertJSON) {
	...
	try {
		...
		if (result.result) {
			...
			showNotification("Insert successfull");
		} else {
			console.log("Failed")
			showNotification("Insert failed", "error");
		}
	} catch (error) {
		console.error("Error:", error);
	}
...
async function selectServlet(selectJSON) {
	...
	try {
		...
		if (result.result) {
			...
			showNotification("Somemthing went wrong!", "error");
		}
	} catch (error) {
		console.error("Error:", error);
	}
...
async function alterServlet(alterJSON) {
	...
	try {
		...
		if (result.result) {
			...
			await tablePrimaryFetch();
			showNotification("Altered successfully");
		} else {
			console.log("ALTER FAILED")
			showNotification("Alter failed", "error");
		}
	} catch (error) {
		console.error("Error:", error);
	}
...
function showDBdeleteUI() {
	document.getElementById("warningPopup").style.display = "flex";
}

document.getElementById("confirmYes").addEventListener("click", function() {
	dropDb();
	console.log("Item deleted"); // Replace with actual delete function
